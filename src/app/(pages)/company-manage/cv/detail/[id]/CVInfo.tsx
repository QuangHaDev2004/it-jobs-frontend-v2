"use client";
import { CVInfoSkeleton } from "@/components/skeleton/CVInfoSkeleton";
import { getCVDetail } from "@/services/company";
import { CVDetail, JobDetail } from "@/types";
import { getJobInfo } from "@/utils/jobHelper";
import { useQuery } from "@tanstack/react-query";
import Link from "next/link";

export const CVInfo = ({ id }: { id: string }) => {
  const { data, isPending } = useQuery({
    queryKey: ["cvDetail", id],
    queryFn: () => getCVDetail(id),
  });
  ...
  if(isPending) return <CVInfoSkeleton />
  ...
  return (
    <>
      ...
      <div className="rounded-lg bg-white p-5 shadow-md">
        ...
        <div className="mb-2.5 text-[16px] font-normal">
          Số điện thoại: <span className="font-bold">{cvDetail?.phone}</span>
        </div>
      ...
      <div className="mt-[20px] rounded-lg bg-white p-5 shadow-md">
        ...
        <div className="mb-2.5 text-[16px] font-normal">
          Tên công việc: <span className="font-bold">{jobDetail?.title}</span>
        </div>
        <div className="mb-2.5 text-[16px] font-normal">
          Mức lương:{" "}
